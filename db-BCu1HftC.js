...
function insertGoalTaskWithMeta(goalId, employeeId, title, meta) {
	return getDb().prepare("INSERT INTO goal_tasks (goal_id, employee_id, task_uid, depends_on_task_uids, deliverable, done_definition, sequence, title) VALUES (?, ?, ?, ?, ?, ?, ?, ?) RETURNING *").get(goalId, employeeId, meta?.taskUid ?? null, meta?.dependsOnTaskUids ? JSON.stringify(meta.dependsOnTaskUids) : null, meta?.deliverable ?? null, meta?.doneDefinition ?? null, meta?.sequence ?? 0, title);
}
function getGoalTasks(goalId) {
	return getDb().prepare("SELECT * FROM goal_tasks WHERE goal_id = ? ORDER BY sequence, id").all(goalId);
}
function getEmployeeActiveTasks(employeeId, goalId) {
	if (goalId !== void 0) return getDb().prepare(`SELECT gt.*, g.title as goal_title
         FROM goal_tasks gt
         JOIN goals g ON g.id = gt.goal_id
         WHERE gt.employee_id = ? AND gt.status != 'done' AND gt.goal_id = ?
         ORDER BY gt.created_at`).all(employeeId, goalId);
...
function markGoalTaskDispatched(id) {
	getDb().prepare("UPDATE goal_tasks SET dispatched_at = datetime('now'), updated_at = datetime('now') WHERE id = ? AND dispatched_at IS NULL").run(id);
}
function updateGoalTask(id, fields) {
	...
	params.push(id);
	getDb().prepare(`UPDATE goal_tasks SET ${sets.join(", ")} WHERE id = ?`).run(...params);
}
function findGoalTaskByTitle(employeeId, titleFragment, goalId) {
	if (goalId !== void 0) return getDb().prepare("SELECT * FROM goal_tasks WHERE employee_id = ? AND goal_id = ? AND title LIKE ? AND status != 'done' LIMIT 1").get(employeeId, goalId, `%${titleFragment}%`);
...
function getPendingDecisions(goalId) {
	if (goalId !== void 0) return getDb().prepare("SELECT * FROM pending_decisions WHERE goal_id = ? ORDER BY created_at DESC").all(goalId);
...
function insertPendingDecision(employeeId, background, optionA, optionB, options, goalId) {
	return getDb().prepare("INSERT INTO pending_decisions (employee_id, goal_id, background, option_a, option_b, options) VALUES (?, ?, ?, ?, ?, ?) RETURNING *").get(employeeId, goalId ?? null, background, optionA, optionB ?? null, options ? JSON.stringify(options) : null);
}
...
function insertDecision(employeeId, summary, choice, context, goalId) {
	return getDb().prepare("INSERT INTO decisions (employee_id, goal_id, summary, choice, context) VALUES (?, ?, ?, ?, ?) RETURNING *").get(employeeId, goalId ?? null, summary, choice, context ?? null);
}
...
function getDecisionsFiltered(opts) {
	...
	return getDb().prepare(`SELECT * FROM decisions ${where} ORDER BY created_at DESC LIMIT ? OFFSET ?`).all(...params);
}
function getDecisionStats(goalId) {
	const rows = goalId !== void 0 ? getDb().prepare("SELECT result_tag, COUNT(*) as cnt FROM decisions WHERE goal_id = ? GROUP BY result_tag").all(goalId) : getDb().prepare("SELECT result_tag, COUNT(*) as cnt FROM decisions GROUP BY result_tag").all();
...
function searchDecisionsByGoal(q, goalId) {
	const like = `%${q}%`;
	return getDb().prepare("SELECT * FROM decisions WHERE goal_id = ? AND (summary LIKE ? OR context LIKE ? OR choice LIKE ?) ORDER BY created_at DESC LIMIT 50").all(goalId, like, like, like);
}
...
function insertReport(employeeId, content, goalId) {
	return getDb().prepare("INSERT INTO employee_reports (employee_id, goal_id, content) VALUES (?, ?, ?) RETURNING *").get(employeeId, goalId ?? null, content);
}
function getRecentReports(limit = 20, goalId) {
	if (goalId !== void 0) return getDb().prepare("SELECT * FROM employee_reports WHERE goal_id = ? ORDER BY created_at DESC LIMIT ?").all(goalId, limit);
...
function getReportsByDays(days, goalId) {
	if (goalId !== void 0) return getDb().prepare("SELECT * FROM employee_reports WHERE goal_id = ? AND created_at >= datetime('now', ? ) ORDER BY created_at DESC LIMIT 200").all(goalId, `-${days} days`);
	return getDb().prepare("SELECT * FROM employee_reports WHERE created_at >= datetime('now', ? ) ORDER BY created_at DESC LIMIT 200").all(`-${days} days`);
}
function getEmployeeReports(employeeId, limit = 10, goalId) {
	if (goalId !== void 0) return getDb().prepare("SELECT * FROM employee_reports WHERE employee_id = ? AND goal_id = ? ORDER BY created_at DESC LIMIT ?").all(employeeId, goalId, limit);
...
function getRecentActivity(limit = 50, goalId) {
	if (goalId !== void 0) return getDb().prepare("SELECT * FROM activity_log WHERE json_extract(meta, '$.goalId') = ? ORDER BY created_at DESC LIMIT ?").all(goalId, limit);
...
function getEmployeeActivity(employeeId, limit = 5, goalId) {
	if (goalId !== void 0) return getDb().prepare("SELECT * FROM activity_log WHERE employee_id = ? AND json_extract(meta, '$.goalId') = ? ORDER BY created_at DESC LIMIT ?").all(employeeId, goalId, limit);
...
function getEmployeeActivityForActiveGoals(employeeId, limit = 5, goalId) {
	...
	return getDb().prepare(`SELECT * FROM activity_log WHERE employee_id = ? AND (meta IS NULL OR json_extract(meta, '$.goalId') IS NULL OR json_extract(meta, '$.goalId') IN (${placeholders})) ORDER BY created_at DESC LIMIT ?`).all(employeeId, ...activeGoalIds, limit);
}
//#endregion
export { updateGoal as A, insertGoalTaskWithMeta as C, searchDecisions as D, markGoalTaskDispatched as E, updateGoalTaskStatus as M, searchDecisionsByGoal as O, insertDecision as S, insertReport as T, getRecentActivity as _, findGoalTaskByTitle as a, insertActivity as b, getDecisionStats as c, getEmployeeActiveTasks as d, getEmployeeActivityForActiveGoals as f, getPendingDecisions as g, getGoalTasks as h, deletePendingDecision as i, updateGoalTask as j, updateDecisionTag as k, getDecisions as l, getGoalById as m, deleteCustomEmployee as n, getActiveGoals as o, getEmployeeReports as p, deleteGoal as r, getCustomEmployees as s, db_exports as t, getDecisionsFiltered as u, getRecentReports as v, insertPendingDecision as w, insertCustomEmployee as x, getReportsByDays as y };
